...
public class VitaminServiceImp {
  private final ConcurrentHashMap<String, Vitamin> vitaminMap = new ConcurrentHashMap<>();

  public Flux<VitaminView> getVitamins() {
    return Flux.fromIterable(vitaminMap
            .values())
  ...
  public Mono<String> getAllVitaminsNames() {
    return Mono.just(String.join(",", vitaminMap.keySet()));
  }

  public Flux<Vitamin> findAll() {
    return Flux.fromIterable(vitaminMap.values());
  }
  ...
  public void initVitamins() {
    ...
    Vitamin d = new Vitamin()
        ...
        .setSources(List.of(
            new Pair()
                .setKey("Sunlight")
                .setValue("The skin can produce vitamin D when exposed to ultraviolet B (UVB) sunlight. However, factors like skin color, geographical location, time of id, and sunscreen use can affect the body's ability to synthesize vitamin D from sunlight."),
            new Pair()
